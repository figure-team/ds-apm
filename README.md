# DS-APM — incident handoff & runbook layer on SigNoz

DS-APM is an **experimental extension layer** built on top of
[SigNoz](https://signoz.io), the open-source, OpenTelemetry-native observability
platform (MIT-licensed). It adds an *incident → SOP runbook → operator handoff*
workflow on top of SigNoz's APM core.

> Built from the SigNoz **community** variant. Enterprise modules (`ee/`,
> `cmd/enterprise/`) are intentionally **not included** — they remain under
> SigNoz's separate Enterprise License and are out of scope for this repository.

### DS-APM extensions

- **SOP grounding & store** — runbook/SOP documents partitioned per org
- **AI runbook drafting** — deterministic, quota-protected strategy with persisted history
- **Notification integration** — SOP/AI payloads wired into native alert channels (Slack / Email / Teams / PagerDuty / Opsgenie / Webhook)
- **PII redaction** — email / KR phone / long-secret scrubbing on incident values
- **Reliability** — JSONL dead-letter sink + idempotent replay ledger

> ⚠️ **Status: early MVP.** Multi-tenant isolation and PII handling are still
> being hardened and are **not production-ready**. Intended for evaluation and demos.

The remainder of this README is the upstream SigNoz documentation.
