...
route:
  ...
  receiver: 'slack'`
	...
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, nil, logger, metrics, nfManager, orgId, nil, nil)
	go dispatcher.Run()
...
func TestDispatcherRace(t *testing.T) {
	...
	dispatcher := NewDispatcher(alerts, nil, nil, marker, timeout, nil, logger, metrics, nfManager, "test-org", nil, nil)
	go dispatcher.Run()
	dispatcher.Stop()
}
...
func TestDispatcher_DoMaintenance(t *testing.T) {
	...
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, nil, promslog.NewNopLogger(), metrics, nfManager, "test-org", nil, nil)
  ...
  receiver: 'slack'`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			d := NewDispatcher(alerts, route, recorder, marker, timeout, nil, logger, metrics, nfManager, "test-org", nil, nil)
