...
type Dispatcher struct {
	...
	// aiHook, when non-nil, runs the DS-APM AI-strategy dispatch hook
	// against each alert before the notify stage, merging SOP metadata and
	// AI strategy annotations into the alert's annotation set. It is
	// intentionally optional: nil keeps pre-DS-APM dispatcher behavior.
...
// run against each alert before the notify stage, populating SOP metadata and
// AI strategy annotations. Pass nil for non-DS-APM callers — behavior is
// identical to the pre-hook form.
func NewDispatcher(
	ap provider.Alerts,
	r *dispatch.Route,
	s notify.Stage,
	...
	go ag.run(func(ctx context.Context, alerts ...*types.Alert) bool {
		if d.aiHook != nil {
			for _, alert := range alerts {
				applyAIHook(ctx, d.aiHook, d.orgID, alert)
			}
		}
...
// applyAIHook runs the DS-APM AI dispatch hook against a single alert and
// merges the resulting annotations back into the alert in place. The hook
// itself is fail-graceful; this wrapper only handles the model.LabelSet ↔
// map[string]string conversion.
//
// Note: IncidentAnnotationIncidentID is not defined in alertmanagertypes as of
// this writing; we use the alert fingerprint string as the incidentID
// throughout.
func applyAIHook(ctx context.Context, hook *dispatchhook.Hook, orgID string, alert *types.Alert) {
	if alert == nil {
		return
	}

	labels := labelSetToStringMap(alert.Labels)
	annotations := labelSetToStringMap(alert.Annotations)
	...
	alert.Annotations = stringMapToLabelSet(merged)
}

func labelSetToStringMap(ls model.LabelSet) map[string]string {
	if ls == nil {
		return nil
	}
...
func stringMapToLabelSet(m map[string]string) model.LabelSet {
	if m == nil {
		return nil
	}
	out := make(model.LabelSet, len(m))
	for k, v := range m {
