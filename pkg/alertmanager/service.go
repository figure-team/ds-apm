...
type Service struct {
	...
	// aiHook, when non-nil, is forwarded to each alertmanagerserver.Server so
	// the DS-APM AI-strategy dispatch hook runs before alert delivery. Nil
	// keeps pre-DS-APM behavior intact.
	aiHook *dispatchhook.Hook
}

// New creates a new alertmanager Service.
//
// aiHook is optional: when non-nil, the DS-APM AI-strategy dispatch hook is
// forwarded to each per-org Server so alerts get their SOP and AI strategy
...
func (service *Service) newServer(ctx context.Context, orgID string) (*alertmanagerserver.Server, error) {
	...
	server, err := alertmanagerserver.New(ctx, service.settings.Logger(), service.settings.PrometheusRegisterer(), service.config, orgID, service.stateStore, service.notificationManager, service.aiHook)
	if err != nil {
		return nil, err
	}
