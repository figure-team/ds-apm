...
// NewFactory constructs the signoz alertmanager provider factory.
//
// aiHook is optional: when non-nil, the DS-APM AI-strategy dispatch hook is
// threaded into every per-org alertmanager Server so alerts receive SOP
// metadata and AI strategy annotations before delivery.
func NewFactory(sqlstore sqlstore.SQLStore, orgGetter organization.Getter, notificationManager nfmanager.NotificationManager, aiHook *dispatchhook.Hook) factory.ProviderFactory[alertmanager.Alertmanager, alertmanager.Config] {
	return factory.NewProviderFactory(factory.MustNewName("signoz"), func(ctx context.Context, settings factory.ProviderSettings, config alertmanager.Config) (alertmanager.Alertmanager, error) {
		return New(ctx, settings, config, sqlstore, orgGetter, notificationManager, aiHook)
	})
}

func New(ctx context.Context, providerSettings factory.ProviderSettings, config alertmanager.Config, sqlstore sqlstore.SQLStore, orgGetter organization.Getter, notificationManager nfmanager.NotificationManager, aiHook *dispatchhook.Hook) (*provider, error) {
