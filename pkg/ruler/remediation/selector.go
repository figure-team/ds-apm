...
func (s *Selector) createExecution(
	...
	ann := map[string]string{
		...
		alertmanagertypes.IncidentAnnotationRemediationApproveURL:    s.approveURL(labels["ruleId"], e.ID),
	}
	return ann, true
}

// approveURL mirrors Proposer.approveURL (same deep link contract).
func (s *Selector) approveURL(ruleID, remediationID string) string {
	p := &Proposer{baseURL: s.baseURL}
	return p.approveURL(ruleID, remediationID)
}
