...
func NewHandlers(
	...
	aiGen ruletypes.AIStrategyGenerator,
	...
	return Handlers{
		...
		RulerHandler: signozruler.NewHandler(signozruler.Deps{
			Ruler:                  rulerService,
			SOPStore:               sqlsopstore.NewSOPStore(sqlStore),
			AIHistoryStore:         sqlaihistorystore.NewAIStrategyHistoryStore(sqlStore),
			AIGenerator:            aiGen,
			AIConfigStore:          aiConfigStore,
			AICipher:               aiCipher,
			AIRebuilder:            storeAware,
			...
			ReportTemplateStore:    sqlincidentreporttemplatestore.New(sqlStore),
			RemediationStore:       remStore,
			RemediationTargetStore: remTargetStore,
			NewRemediationExecutor: newExec,
			RemediationHealth:      remHealthChecker,
		}),
	}
}

// newAIConfigStore builds an AIConfigStore from the supplied SQLStore.
// Exported for use by signoz.go wiring.
func newAIConfigStore(sqlStore sqlstore.SQLStore) ruletypes.AIConfigStore {
	return sqlaiconfigstore.New(sqlStore)
}
