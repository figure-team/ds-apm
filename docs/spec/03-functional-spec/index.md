...
본 문서는 AIOpsAgent가 SigNoz community 빌드 위에서 **Incident → SOP runbook → Operator handoff** 흐름을 자동화하기 위해 요구하는 기능·비기능 요구사항을 정의해야 한다. 산출물은 ISO/IEC/IEEE 29148:2018의 SRS 템플릿을 POC 단계에 맞게 경량화(`§1, §2, §3, §4, §5`)한 형태이며, 각 기능 요구사항은 Specification by Example의 Living Documentation 원칙에 따라 Gherkin acceptance criteria로 보강된다 (각 모듈 `F*.md` §F*.7).
...
### §1.4 Product Scope
- **In Scope**: AIOpsAgent (incident handoff, SOP grounding, AI runbook, notification dispatch, PII redaction, DLQ/replay) — F0~F8 9개 모듈 전부.
...
### §1.5 References
- [`../_foundation/baseline.md`](../_foundation/baseline.md) — 사전 PoC 단계의 변경 표면 + 커밋 ↔ 모듈 매핑 (내부 audit 자료)
...
**AIOpsAgent**는 SigNoz Community 빌드의 알림 처리 경로에 운영 자동화(SOP 그라운딩·AI 초안·DLQ 재처리) 단계를 추가하는 확장 모듈 그룹이다. SigNoz의 `pkg/alertmanager/`, `pkg/ruler/`, `pkg/types/ruletypes/`, `cmd/community/`에 통합된 Go-native MVP이며, SigNoz 없이는 동작하지 않는다 (ADR-001).

| 의존 대상 | 역할 | 변경 가능성 |
|---|---|---|
| **SigNoz Ruler** | alert evaluation + firing | upstream API 변경 시 AIOpsAgent 영향 — 모니터링 필요 (R-6) |
| **SigNoz Alertmanager (`dispatch.Dispatcher`)** | dispatcher hot path | F6은 `dispatch.Dispatcher`를 wrapping해서 `aiHook` + `dlqSink`를 연결 |
| **SigNoz `notify.Stage`** | 채널별 notification stage | 5채널 어댑터 (Slack / Teams v2 / PagerDuty / Webhook / Email)를 패치 |
| **SigNoz ClickHouse 연결** | observability backend | AIOpsAgent는 metrics/logs sink로만 활용, 직접 schema 변경 안 함 |

fork 프레이밍 **금지** — AIOpsAgent는 SigNoz의 fork가 아니라 같은 코드라인 위의 확장이다 (메모리 정책 "var/signoz는 우리 코드").
...
| **Storage (Observability)** | ClickHouse — SigNoz upstream 그대로. AIOpsAgent는 schema 변경 안 함. |
...
N/A — AIOpsAgent는 컨테이너 software-only.

### §4.3 Software Interfaces

| 인터페이스 | 방향 | 프로토콜 / API | 비고 |
|---|---|---|---|
| **SigNoz Ruler API** | AIOpsAgent ↔ Ruler | Go internal — `pkg/apiserver/signozapiserver/ruler.go` (+182), `pkg/ruler/signozruler/handler.go` (+467) | F1, F2 wiring |
| **SigNoz Alertmanager (`dispatch.Dispatcher`)** | AIOpsAgent → Dispatcher hot path | Go internal — `pkg/alertmanager/alertmanagerserver/dispatcher.go` wrapping (+56) | F6, F8 |
...
| **ClickHouse** | AIOpsAgent → CK | SigNoz upstream observability path 그대로 (metrics, logs) | schema 변경 없음 |
| **PostgreSQL (bun ORM)** | AIOpsAgent ↔ DB | bun ORM, table `ds_sop_documents` (마이그레이션 078). 복합 키 `(org_id, sop_id, version)` | F1 SOP store |
...
| **COMM-OUT-3** | PagerDuty | Events API v2 (`routing_key`, `event_action: trigger`, `dedup_key`, `payload`) | `dedup_key`는 AIOpsAgent의 idempotency_key와 1:1 매핑 |
...
- **NF-5.5.3 (Handoff acknowledgment)** 운영자 교대 시 명시적 인수와 firm acknowledgment 수신 전까지 콜에서 나가지 않아야 한다 (PagerDuty Incident Command 모델, research §1.2). 본 룰은 운영 프로세스 권고로, AIOpsAgent는 audit trail로만 지원.
...
| F0~F8 ↔ WBS-1.0~1.5 (커밋 매핑은 traceability.md §4 참조) | traceability.md §4 (역사 추적) |
