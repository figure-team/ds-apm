...
jtbd: [JTBD-1, JTBD-3]
maps_modules: [F1, F4]
commits: [72944ecac, c7f4fd330, 3fa604e03]
source_paths:
...
- **Acceptance**:
  ...
  ```
- **구현 근거**: `sop_document.go: PreviewSOPDocumentBinding` — alert label `signoz_pilot_sop_id`로 explicit-label binding(`status=bound`, `resolution=explicit_label`). vector retrieval 미도입. · `72944ecac` · WBS-1.1
...
- **Acceptance**:
  ...
  ```
- **구현 근거**: `SOPStore`(sqlsopstore, bun ORM, 마이그레이션 078 `ds_sop_documents`, `(org_id, sop_id, version)` 키) — `Upsert/Get/GetLatest/List`. 무결성: `sha256:` checksum · 본문 256 KiB 상한 · markdown 형식 검증 · `safeDisplayURL`(http/https만, sensitive query 제거). ⚠️ `GetLatest`는 version 문자열 DESC 정렬이라 zero-pad(`v01`) 필요. · `c7f4fd330` · WBS-1.1
...
- **Acceptance**:
  ...
  ```
- **구현 근거**: `tenant_policy.go: PilotTenantScopeAllows`(정확 일치 또는 `*`), `SOPBindingStatusForbidden`, `SOPTenantPolicyDeniedWarning`. label 누락 시 `PilotTenantIsComplete=false`로 deny. · `3fa604e03` · WBS-1.0
...
- **Acceptance**:
  ...
  ```
- **구현 근거**: cross-tenant `Get`은 `ErrSOPDocumentNotFound`로 통일(존재 누설 금지). → NF-5.3.4. · `3fa604e03` · WBS-1.0
...
- **Acceptance**:
  ...
  ```
- **구현 근거**: `approvalStatus=disabled` → `SOPBindingStatusDisabled` + "sop document is disabled" 경고. staleness 90일 정책 → NF-5.5.3. · `72944ecac` · WBS-1.1
