...
- **Acceptance**:
  ...
  ```
- **구현 근거**: `knownIncidentTemplateFields` 22종(tenant/impact/sop/ai 그룹), `PreviewNotificationTemplate`, `MissingIncidentTemplateVariables`(prefix-qualified·정렬). · `5c036c806` · WBS-1.3
...
- **Acceptance**:
  ...
  ```
- **구현 근거**: `recordTerminalFailure` → `dlqSink.Write`(best-effort). `ctx.Canceled`는 `DebugContext`(정상 경로), terminal error만 DLQ. `aiHook==nil`이면 hook 건너뜀(AIOpsAgent 미설치 동작). → NF-5.2.4 · `5c036c806` · WBS-1.3
