...
scope: hybrid (구현 핵심 CF-1~6 상세 + CF-7 1차 설계 상세(planned) + 로드맵 후속 저fidelity)
maps_modules: [F0, F1, F2, F3, F4, F5, F6, F7, F8, F9]
updated: 2026-06-11
---
...
### UJ-4 사전대응·자산화 *(로드맵)* — CF-7후속·8·9
이상 징후 사전 경고(CF-7 후속: 기준선 학습) → 승인 기반 자동조치(CF-8) → 자동 보고서·지식 자산화(CF-9). 미구현(§6·§9.3).

### UJ-5 미등록 예외 (인지·분석·자산화) *(planned, 설계 확정)* — CF-7(1차)·3·4·5·6
- **트리거**: Alert Rule에 등록되지 않은 신규 예외가 텔레메트리(트레이스 예외·에러 로그)에 출현. **Persona**: 운영자·관리자.
- **경로**: 스캐너가 시그니처 정규화로 신규 클러스터 인지 → **1차 알림 즉시**(synthetic alert → 기존 5채널·PII·DLQ·감사 재사용) → 비동기 분석(스택트레이스 → 로컬 미러 git/svn 코드 컨텍스트 → LLM) → 보고서 초안 저장 + **2차 알림**(요약+링크) → 운영자 검수 → SOP/Runbook **draft** 등록(승인은 기존 상태머신) → 재발 시 UJ-1 골든패스로 흡수.
- **분기**: 분석·LLM 실패 → 1차 알림 이미 전달(정보 손실 0, UJ-3 계승) · 코드전송 스위치 OFF → 로그 기반 degrade · dismiss → 재알림 영구 차단 · 동일 시그니처 재발 → 억제 윈도우 내 count만 갱신.
- **결과**: 미등록 예외도 인지·분석·자산화. **최소보장**: 1차 알림은 분석과 무관하게 전달(silent drop 0). 상세: [features/CF-7-unknown-exception.md](features/CF-7-unknown-exception.md).
...
| **CF-7** | 미등록 예외 대응 (이상 탐지 1차) — 인지·코드연계 분석·자산화 | 1,2,4,5 | ◑ planned (설계 확정) | 2단계 | F9 | [features/CF-7-unknown-exception.md](features/CF-7-unknown-exception.md) |
...
### §7.2 CF-7 1차 (미등록 예외 대응) — 설계 확정 (planned)

> FR 상세(무엇을/Acceptance/구현 계획)는 [features/CF-7-unknown-exception.md](features/CF-7-unknown-exception.md). 설계 원천: [설계서](../../superpowers/specs/2026-06-11-unknown-exception-response-design.md).

| FR | 요구 (고객 voice) | JTBD | UJ | WBS |
|---|---|---|---|---|
| FR-CF7.2 | 운영자는 Alert Rule에 등록되지 않은 신규 예외가 발생하면 시스템이 자동 인지해 알림을 받는다 — 등록된 현상만 기다리지 않는다 | 1 | UJ-5 | WBS-1.6 |
| FR-CF7.3 | 운영자는 신규 예외에 대해 코드베이스(git/svn) 근거(소스·변경이력)가 포함된 원인 분석 보고서 초안을 받는다 | 2 | UJ-5 | WBS-1.6 |
| FR-CF7.4 | 관리자는 분석에 사용할 저장소 연결과 소스코드 외부 전송 허용 여부를 설정으로 통제한다 | 6 | UJ-5 | WBS-1.6 |
| FR-CF7.5 | 운영자는 보고서를 검토해 신규 SOP/Runbook으로 등록할 수 있고, 등록물은 승인 전 효력이 없다(draft, HITL) | 4,5 | UJ-5 | WBS-1.6 |
| FR-CF7.6 | 운영자는 동일 예외의 반복 알림에 시달리지 않으며, 무시 처리한 예외는 다시 알림되지 않는다 | 1 | UJ-5 | WBS-1.6 |
| FR-CF7.7 | 운영자는 분석·AI가 실패해도 신규 예외 발생 알림 자체는 누락 없이 받는다 (fail-open) | 4 | UJ-5 | WBS-1.6 |

### §7.3 로드맵 (CF-7 후속~CF-10) — 저fidelity FR + 선행

| FR | 요구 (고객 voice) | JTBD | 단계 | 선행 |
|---|---|---|---|---|
| FR-CF7.1 *(CF-7 후속 — 1차 범위 아님)* | 운영자는 비정상 임계치를 사전 경고로 받는다 (AI 기준선 학습) | 1 | 2단계 | OTel·데이터 학습 |
...
- **CF-7 1차 (미등록 예외 대응)** — 설계 확정·구현 착수 전 (§7.2, WBS-1.6, Epic 7).
- **CF-7 후속~CF-10** (기준선 학습·자동조치·자동보고서/지식화·ITSM) — 단계 §6 참조.
