...
### UJ-5 코드 RCA (unbound∧anomaly 코드 근본원인 분석) *(implemented-mvp)* — CF-11 (+CF-7 트리거·CF-1·3·6 재사용)
...
incident 생명주기 순. **CF-1~6 = 구현 핵심(★)**, **CF-7 이상탐지·CF-11 코드RCA = 구현(★/◑mvp)**, **CF-8~10 = 전략 로드맵(○ 미구현)**.
...
| **CF-11** | AI 코드베이스 RCA — unbound∧anomaly 코드 근거 근본원인 분석 (HITL) | 1,2,4 | ◑ implemented-mvp | 2단계 | F10 | [features/CF-11-code-rca.md](features/CF-11-code-rca.md) |
...
> CF-11 코어(M1~M3)는 TDD 완료, HTTP·FE·디스패치 훅 트리거는 통합 seam(설계 §11) → status=implemented-mvp.
...
- **CF-11 (AI 코드베이스 RCA)** — 코어(M1~M3) 구현·TDD 완료, **HTTP·FE·디스패치 훅 트리거·서버 배선은 통합 seam**(설계 §11). (§7.2, WBS-1.7, Epic 11).
