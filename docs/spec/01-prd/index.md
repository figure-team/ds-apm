...
scope: hybrid (구현 핵심 CF-1~6 + CF-7 이상탐지 + CF-11 코드RCA 상세 + 로드맵 CF-8~10 저fidelity)
maps_modules: [F0, F1, F2, F3, F4, F5, F6, F7, F8, F9, F10]
updated: 2026-06-14
---
...
> **범위(하이브리드)**: 실제 코드로 구현된 핵심(CF-1~6) + 이상 탐지(CF-7) + AI 코드베이스 RCA(CF-11)는 FR 상세 + Given/When/Then까지. 전략 로드맵의 미구현 역량(CF-8~10, CF-7 학습형 후속)은 저fidelity FR + 단계 태그만.
...
### UJ-4 사전대응·자산화 *(로드맵)* — CF-7 학습형 후속·8·9
이상 징후 사전 경고(CF-7 학습형 후속: 계절성 기준선 학습) → 승인 기반 자동조치(CF-8) → 자동 보고서·지식 자산화(CF-9). 미구현(§6·§9.3).

### UJ-5 코드 RCA (unbound∧anomaly 코드 근본원인 분석) *(implemented)* — CF-11 (+CF-7 트리거·CF-1·3·6 재사용)
- **트리거**: 연계 SOP가 없고(unbound) 이상(anomaly, CF-7)이 감지된 알람. **Persona**: 운영자.
- **경로**: 디스패치 경로 seam → coderca 게이트(기능ON ∧ unbound ∧ anomaly ∧ 심각도 ∧ 서비스→저장소 매핑 ∧ admission[dedup·예산·큐]) → 영속 `coderca_run` 적재(훅 즉시 반환) → 워커 풀 claim(lease·동시성 캡) → 서비스→저장소 해석 → 소스 fetch + **기준 커밋 고정** → CLI 코딩 에이전트(claude/codex, **read-only**) 코드 탐색 → 근본원인+수정 제안 파싱 → 핸드오프(CF-3) 전달 + 이력 + 감사(CF-6).
- **분기**: anomaly 부재 → 미발화(fail-closed) · 폭주 → dedup/예산/큐 skip(카운터만) · 분석 실패·타임아웃·비용초과 → status=failed/timeout, **알람 전달 무영향**(UJ-3 계승) · 암호화 키 부재+비공개 repo → fail-closed.
- **결과**: 운영자가 코드 근거 근본원인 + 수정 **제안**(검토용)을 받음. 수정은 절대 자동 적용 안 됨(HITL). **최소보장**: CF-11의 어떤 실패도 알람 경로를 막지 않음(silent drop 0). 상세: [features/CF-11-code-rca.md](features/CF-11-code-rca.md).

---

## §6. Feature Map

incident 생명주기 순. **CF-1~6 = 구현 핵심(★)**, **CF-7 이상탐지·CF-11 코드RCA = 구현(★)**, **CF-8~10 = 전략 로드맵(○ 미구현)**.
...
| **CF-7** | 메트릭 이상 탐지 (Anomaly Detection — z-score 기준선) | 1 | ★ implemented | 2단계 | F9 | [features/CF-7-anomaly.md](features/CF-7-anomaly.md) |
...
| **CF-11** | AI 코드베이스 RCA — unbound∧anomaly 코드 근거 근본원인 분석 (HITL) | 1,2,4 | ★ implemented | 2단계 | F10 | [features/CF-11-code-rca.md](features/CF-11-code-rca.md) |
...
### §7.2 CF-7 이상 탐지 + CF-11 AI 코드베이스 RCA — 구현

> CF-7 FR 상세: [features/CF-7-anomaly.md](features/CF-7-anomaly.md) (`anomaly_rule.go`). CF-11 FR 상세(무엇을/Acceptance/구현 근거): [features/CF-11-code-rca.md](features/CF-11-code-rca.md). 설계 원천: [CF-11 설계서](../../superpowers/specs/2026-06-11-cf11-code-rca-design.md).

| FR | 요구 (고객 voice) | JTBD | UJ | WBS |
|---|---|---|---|---|
| FR-CF7.1 *(implemented)* | 운영자는 메트릭이 통계 기준선(이동평균±k·σ)을 벗어나면 이상 알람을 받는다 — 고정 임계치를 매번 튜닝하지 않는다 | 1 | UJ-1·5 트리거 | WBS-1.6 |
| FR-CF11.1 | 운영자는 매칭 SOP가 없고(unbound) 이상(anomaly)인 알람에 대해 AI 에이전트가 코드를 탐색해 만든 근본원인+수정 제안 보고서를 받는다 | 2 | UJ-5 | WBS-1.7 |
| FR-CF11.2 | 관리자는 분석 대상 저장소 연결·서비스 매핑·기능 on/off를 설정으로 통제한다(자격증명 암호화·키 부재 시 fail-closed) | 6 | UJ-5 | WBS-1.7 |
| FR-CF11.3 | 운영 조직은 동일 오류 폭주가 비용·볼륨 폭발로 이어지지 않음을 보장받는다(dedup·예산·동시성 캡) | 4 | UJ-5 | WBS-1.7 |
| FR-CF11.4 | 운영자는 AI 수정이 자동 적용되지 않고 read-only 제안으로만 전달됨을 보장받는다(HITL) | 4 | UJ-5 | WBS-1.7 |
| FR-CF11.5 | 운영자는 분석이 수행된 기준 커밋(baseline)을 명시받는다 | 2 | UJ-5 | WBS-1.7 |
| FR-CF11.6 | 운영자는 분석이 실패·타임아웃해도 알람 전달이 영향받지 않음을 보장받는다(fail-open) | 4 | UJ-5 | WBS-1.7 |

> CF-11 코어(M1~M3) TDD 완료 + 전체 통합 seam(디스패치 훅 트리거·HTTP·FE·서버 배선) 배선·테스트 완료 → status=implemented. 잔여 follow-up(EvidenceCollector·OS 샌드박스·monorepo clone·수동 smoke)은 CF-11 open_items.

### §7.3 로드맵 (CF-7 학습형 후속·CF-8~10) — 저fidelity FR + 선행

| FR | 요구 (고객 voice) | JTBD | 단계 | 선행 |
|---|---|---|---|---|
| FR-CF7.2 *(CF-7 학습형 후속)* | 운영자는 계절성·시간대를 학습한 기준선으로 사전 경고를 받는다 (AI 이상 탐지 모델 학습) | 1 | 2단계 | OTel·데이터 학습 |
...
- **CF-11 (AI 코드베이스 RCA)** — **구현 완료**(코어 + 전체 통합 seam 배선·테스트). 잔여 follow-up만 open: EvidenceCollector(logs/traces, 현재 Noop)·OS 샌드박스 래핑·대형 monorepo shallow clone·수동 end-to-end smoke(라이브 서버). (§7.2, WBS-1.7, Epic 11, CF-11 open_items).
- **CF-7 학습형 후속·CF-8~10** (계절성 기준선 학습·자동조치·자동보고서/지식화·ITSM) — 단계 §6 참조. (CF-7 v1 통계 이상탐지는 구현됨.)
...
- [`../_shared/traceability.md`](../_shared/traceability.md) — CF×UJ×WBS (CF 축, 번호 정합 완료 — CF-7 이상탐지·CF-11 코드RCA)
