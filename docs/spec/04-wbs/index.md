...
├─ WBS-1.0  Foundation                — pilot contract, managed markdown, audit sink, tenant policy
│           Covers: F0, F4, F5
├─ WBS-1.1  SOP Engine                — SOP store, grounding, file persistence
│           Covers: F1
├─ WBS-1.2  AI Drafter                — runbook drafter, AI generator, quota controls, strategy history
│           Covers: F2, F3
├─ WBS-1.3  Notification Dispatcher   — 5 채널 adapter + dispatcher
│           Covers: F6
├─ WBS-1.4  PII Redactor              — incident payload redaction
│           Covers: F7
└─ WBS-1.5  DLQ + Replay              — JSONL DLQ, idempotent replay ledger
            Covers: F8
```
...
|---|---|---|---|---|
| WBS-1.0 | Foundation | implemented | F0, F4, F5 | [WBS-1-0-foundation.md](packages/WBS-1-0-foundation.md) |
| WBS-1.1 | SOP Engine | implemented | F1 | [WBS-1-1-sop-engine.md](packages/WBS-1-1-sop-engine.md) |
| WBS-1.2 | AI Drafter | implemented | F2, F3 | [WBS-1-2-ai-drafter.md](packages/WBS-1-2-ai-drafter.md) |
| WBS-1.3 | Notification Dispatcher | implemented | F6 | [WBS-1-3-notification-dispatcher.md](packages/WBS-1-3-notification-dispatcher.md) |
| WBS-1.4 | PII Redactor | implemented (MVP) | F7 | [WBS-1-4-pii-redactor.md](packages/WBS-1-4-pii-redactor.md) |
| WBS-1.5 | DLQ + Replay | implemented (HMAC pending) | F8 | [WBS-1-5-dlq-replay.md](packages/WBS-1-5-dlq-replay.md) |

## Excluded Scope (명시적 OUT OF SCOPE)

- **SigNoz upstream 기능 자체** — DS-APM은 위에 얹은 layer
...
|---|---|---|---|
| **M-1 POC 완료** | 11 commits / +12,632 LOC 흡수, F0~F8 9 모듈 전부 `status: implemented`, UC-001~003 본문·시퀀스·NFR 작성. | **달성** (2026-05-29 산출물 기준) | WBS-1.0 ~ WBS-1.5 전부 |
...
## Appendix
- [Phase 시간선 (P0~P5)](appendix-phases.md) — 11 커밋의 시간 순 분할 (역사 추적용)
