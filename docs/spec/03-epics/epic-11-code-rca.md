...
> **상태**: implemented — 코어(M1~M3) TDD 완료 + HTTP·FE·디스패치 훅 트리거·서버 배선 등 전체 통합 seam 배선·테스트 완료(설계 §11). 잔여 follow-up(EvidenceCollector·OS 샌드박스·monorepo clone·수동 smoke)은 CF-11 open_items.

## 스토리 (별도 파일)

| 스토리 | 제목 | FR | 상태 |
|---|---|---|---|
| [Story 11.1](../04-stories/11.1.story.md) | 트리거 게이트 + AI 코드 RCA 보고서 | FR-CF11.1 | done |
| [Story 11.2](../04-stories/11.2.story.md) | 저장소·서비스 매핑·기능 토글 설정 | FR-CF11.2 | done |
...
| [Story 11.6](../04-stories/11.6.story.md) | 분석 실패에도 알람 무영향 (fail-open) | FR-CF11.6 | done |
