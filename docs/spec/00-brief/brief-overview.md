...
| 목표 | 의미 | 측정 기준 |
|---|---|---|
| **무손실** | 어떤 실패 상황에서도 운영 담당자는 알람과 SOP를 수신합니다. silent drop 없음. | drop 건수 0 |
| **30초 안 발송** | 장애 발생 → 메신저 도달까지 평균 30초 이내 (운영 담당자 승인 시간 제외) | p95 지연 ≤ 30초 |
| **실행 이력 감사** | 누가 언제 무엇을 수행했는지 모두 영속 기록 | 감사 누락률 0% |
...
| **R-4** | 외부 LLM 의존 | **이미 해결됨** — LLM 실패 시 SOP 원문 그대로 전달(장애 시 통과(fail-open)). 무손실 보장 |
...
| **상세본 Overview** | arc42 v9.0 전체 (아키텍처·품질 시나리오) | [../01-overview/index.md](../01-overview/index.html) |
...
> 추가 문의는 [한장 브리핑](brief.html) 또는 [상세본 Overview](../01-overview/index.html) 참조.
