...
# DS-APM 작업 분해 (팀장용 WBS(Work Breakdown Structure, 작업 분해 구조))
...
- **개발자용 산출물(Deliverable) 4종 착수 후 순차 작성 예정** — Overview · Use Case · 기능명세서 · WBS.
...
| WBS-1.1 SOP 그라운딩 서비스 (SOP Grounding Service) | 운영 절차서(SOP, Standard Operating Procedure, 운영 절차서)를 저장하고 알람과 매칭 | 착수 예정(planned) |
...
- **산출물(Deliverable)**: SOP(Standard Operating Procedure, 운영 절차서) 저장소, SOP 등록·조회 API, alert 라벨(`alertname`·`runbook_url`)로 SOP를 자동 매칭하는 grounding(연결) 로직, 파일 영속화 구현.
...
- **산출물(Deliverable)**: LLM(Large Language Model, 대규모 언어 모델, 외부 AI Provider)으로 조치 초안을 생성하는 runbook drafter 모듈, AI 호출 추상화, 호출 이력·전략 영속화, 쿼터(quota) 제어(장애 시 통과(fail-open)). 발송 직전에 AI 초안을 합치는 dispatch hook.
...
|---|---|---|
| 비개발자 한장 브리핑 | 가장 짧은 요약 (5분) | [brief.html](brief.html) |
| WBS 본문 (개발자용) | 컴포넌트별 산출물(Deliverable) / 인수 기준(Acceptance) / 선행 작업(Dependencies) | [wbs.html](../04-wbs/wbs.html) |
| Phase 시간선 (이력) | Phase별 작업 순서 분할 | [appendix-phases.md](../04-wbs/appendix-phases.md) |
| Use Case | 정상 흐름 + 실패 시나리오 2건 | [usecase.html](../02-usecase/usecase.html) |
| 기능명세서 | 모듈별 인터페이스 + 인수 테스트 | [functional-spec.html](../03-functional-spec/functional-spec.html) |

---

> 본 브리핑은 PMI(Project Management Institute, 미국 PM 표준 기구) 표준 WBS(Work Breakdown Structure, 작업 분해 구조)의 개발자 버전을 비기술 청중용으로 압축한 것입니다.
