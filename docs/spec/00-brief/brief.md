...
| **다음 단계** | M-2 Production Readiness — 일정은 팀장 의사결정 후 확정 |
...
**핵심**: **정보 손실 0**. 발송 실패라고 알람이 누락되지 않습니다.
...
**대응**: M-2 단계에서 결정합니다. **§10 D-1 팀장 의사결정 필요**.
...
**대응**: M-2에서 강화합니다. 외부 고객사 온보딩 일정에 맞춰 우선순위를 조정합니다.
...
| 목표 | 의미 | 측정 기준 |
|---|---|---|
| **정보 손실 0** | 어떤 실패에서도 운영 담당자는 알람과 SOP를 받습니다. 무음 누락(silent drop) 없음. | 누락 건수 0 |
| **30초 안 발송** | 장애 발생 → 메신저 도달까지 평균 30초 이내 (운영 담당자 승인 시간 제외) | p95 지연시간(latency) ≤ 30초 |
| **100% 감사 기록** | 누가 언제 무엇을 했는지 모두 영속 기록 | 감사 누락률 0% |
...
| 마일스톤 | 시점 | 산출물 |
|---|---|---|
| **M-1** | TBD (팀장 결정) | 착수 예정(planned) — 6개 컴포넌트 구축 · 핵심 기능 흐름 확보 |
| **M-2** | TBD (팀장 결정) | Production Readiness — HMAC 결정 + 테넌트 격리 + PII 강화 |
| **M-3** | TBD | Beta 외부 운영 담당자 공개 |

M-2를 시작하려면 다음 의사결정이 필요합니다.
...
| # | 사안 | 옵션 |
|---|---|---|
| **D-1** | HMAC 정책 | (a) M-2 1순위로 즉시 도입 / (b) 외부 고객사 온보딩 직전에 도입 / (c) 보안팀과 별도 검토 진행 |
| **D-2** | 다중 테넌트 격리 강화 시점 | (a) M-2에 포함 / (b) 외부 고객사 1호 확정 후 / (c) 내부 단일 테넌트만 운영 시 보류 |
