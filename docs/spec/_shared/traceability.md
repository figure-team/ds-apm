...
>
> **2026-06-12 번호 정합 (통합)**: 우리 구현 번호로 정렬했다. **CF-7 = 메트릭 이상 탐지**(`anomaly_rule.go`, FR-CF7.1, implemented), **CF-11 = AI 코드베이스 RCA**(`pkg/ruler/coderca`, implemented-mvp, 마이그레이션 082). jinhyeok 브랜치가 코드 RCA를 "CF-7 미등록 예외 대응"으로 표기했던 충돌을 해소 — 해당 스캐너 원안은 [CF-11 §부록(이연 대안)](../01-prd/features/CF-11-code-rca.md)에 보존.
...
| CF-11 | AI 코드베이스 RCA *(implemented-mvp)* | | | | | ✓ |
...
| UJ-2 실패·복구 | ✓ | | | ✓ | | ✓ | | |
| UJ-3 fail-open | ✓ | | ✓ | | | | | |
| UJ-5 코드 RCA *(implemented-mvp)* | | | | | | | (트리거) | ✓ |
...
| CF-11 *(implemented-mvp)* | JTBD-1, 2, 4 | UJ-5 |

> UJ-1=골든(정상), UJ-2=DLQ 실패·복구, UJ-3=LLM fail-open, UJ-4=사전대응·자산화(로드맵), UJ-5=코드 RCA(unbound∧anomaly→AI 코드 분석→HITL, implemented-mvp). 상세 내러티브: [`../01-prd/index.md`](../01-prd/index.md) §5.
...
| F10 *(implemented-mvp)* | AI codebase RCA (`pkg/ruler/coderca`: `engine`·`clirunner`·`runstore`·`sourcestate`·`reporesolver`·`codebaseconfigstore`) | CF-11 | WBS-1.7 |

> 마이그레이션: 078(`ds_sop_documents`·`ds_ai_strategy_history`), 079(`ds_ai_config`), 080(oauth 컬럼), 081(`alter ds_ai_strategy_history multi`), **082**(`082_add_ds_codebase_config.go` — `ds_codebase_repo`·`ds_codebase_service_map`·`ds_codebase_config`·`coderca_run`·`coderca_admission`·`coderca_budget`·`coderca_capacity`·`coderca_skip_stat`, F10/coderca). 이상 탐지(F9)는 **신규 테이블 없이** 기존 rule 엔진을 확장(rule JSON의 anomaly 타입). F9/F10은 [`component-source-map.md`](component-source-map.md)에 as-built 반영 필요(follow-up).
...
| CF-11 코드 RCA *(implemented-mvp, surface seam open)* | FR-CF11.1~11.6 | 6 |
...
| 11 | [11.1](../04-stories/11.1.story.md) | 트리거 게이트 + AI 코드 RCA 보고서 | FR-CF11.1 | done (코어 · 디스패치 seam) |
| 11 | [11.2](../04-stories/11.2.story.md) | 저장소·서비스 매핑·기능 토글 설정 | FR-CF11.2 | done (store·secretbox · HTTP/FE seam) |
...
| 11 | [11.6](../04-stories/11.6.story.md) | 분석 실패에도 알람 무영향 (fail-open) | FR-CF11.6 | done (코어 · e2e는 seam) |

> Epic N = CF-N(1:1). **Epic 7 = CF-7(이상 탐지), Epic 11 = CF-11(코드 RCA).** Story FR = PRD §7 인수 기준. 코드 근거는 각 story Dev Notes. Epic↔WBS 컴포넌트는 §2. CF-11 코어(M1~M3)는 TDD 완료, **HTTP·FE·디스패치 훅 트리거는 통합 seam**(설계 §11, 미배선) → CF-11 status=implemented-mvp.
...
| CF-11 통합 seam (디스패치 훅 트리거·라우트 등록·서버 배선·FE) | open | CF-11 | 코어 구현·테스트 완료, 배선만 통합 단계 (설계 §11) |
| CF-11 HTTP 핸들러 + 설정 FE | open | CF-11 | M4(설계 §13) — 후행 |
