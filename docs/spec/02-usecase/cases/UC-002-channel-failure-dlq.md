...
> **상태**: 구현 완료
> 메신저/페이저 webhook이 4xx/5xx로 응답할 때 retry → DLQ → 운영자 manual replay 흐름. UC-001 단계 8의 Extension `8a`로부터 진입한다.
...
- **DLQ 저장 백엔드**: JSONL 파일 sink (현재 구현) / Redis Streams (미구현) / Kafka topic (미구현).
- **Replay 트리거**: UI 버튼 / CLI 명령 (현재 구현) / 자동 스케줄 replay (미구현).
...
## Traceability
- **Implements features**: F6 (Notification Dispatch), F8 (DLQ + Replay)
