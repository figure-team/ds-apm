...
> 사업 전략서([`../_foundation/source-strategy-brief.md`](../_foundation/source-strategy-brief.md))의 단계별 로드맵을 WBS 컴포넌트(구현)·CF·로드맵 역량에 매핑한다. WBS 본문(8 컴포넌트)은 전략 **1~2단계의 SOP/알림 + 이상탐지·코드RCA 슬라이스**에 해당하며, 그 이후 단계는 로드맵(CF-8~10, CF-7 학습형 후속)이다.
...
| | | **메트릭 이상 탐지** (통계 z-score 기준선) | **WBS-1.6 / CF-7** | ★ 구현 |
| | | **SOP 없는 이상 알람의 AI 코드베이스 RCA** (코드 근거 근본원인 분석) | **WBS-1.7 / CF-11** | ◑ impl-mvp (코어·seam) |
| | | AI 이상 탐지 모델 학습(계절성 기준선) | **CF-7 학습형 후속** | ○ planned |
...
- **2단계 후반 이상 탐지(CF-7, WBS-1.6)는 통계 기반(z-score)으로 구현**([Epic 7](../03-epics/epic-7-anomaly.md)), 그 위에 **SOP 없는 이상 알람의 AI 코드베이스 RCA(CF-11, WBS-1.7)가 구현-mvp**(코어 완료, 표면·디스패치 훅 트리거 seam — [Epic 11](../03-epics/epic-11-code-rca.md)). 학습형 기준선(FR-CF7.2)은 후속. **3단계(자동조치 CF-8)·별첨1(자산화 CF-9·ITSM CF-10)** 은 미구현 로드맵으로, 기능명세 §6/§9.3과 본 부록에만 명시한다 (저fidelity).
